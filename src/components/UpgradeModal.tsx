...
const UpgradeModal = ({ isOpen, onClose }: UpgradeModalProps) => {
  const proFeatures = [
    ...
    {
      icon: <Image className="h-3 w-3 sm:h-4 sm:w-4" />,
      title: "HD Thumbnail Designs",
      description: "Professional thumbnail concepts and mockups"
    },
    {
      icon: <History className="h-3 w-3 sm:h-4 sm:w-4" />,
        ...
        <div className="space-y-3 sm:space-y-4">
          <div className="text-center">
            <div className="text-xl sm:text-2xl font-bold">
              $6<span className="text-xs font-normal text-muted-foreground">/month</span>
            </div>
            ...
            {proFeatures.map((feature, index) => (
              <div key={index} className="flex items-start gap-2 p-2 rounded-lg bg-muted/30">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="font-medium text-xs sm:text-sm truncate">{feature.title}</h4>
          ...
          <div className="bg-gradient-to-r from-primary/10 to-accent/10 p-3 rounded-lg">
            <h3 className="font-medium mb-1 text-xs sm:text-sm">🚀 Perfect for Content Creators</h3>
